...
import { useManagerDashboardActivity } from "@/hooks/managerdashboard.hook";
...
const WorkOrderActivity = () => {
  ...
  const granularityMap = { Yearly: "year", Monthly: "month", Weekly: "week" };
  const { data: activity, isLoading: activityLoading } =
    useManagerDashboardActivity({ granularity: granularityMap[timeframe] });
  const ordersChartData = useMemo(() => {
    const pts = activity?.points || [];
    return pts.map((p) => ({ label: p.label, orders: p.count }));
  }, [activity]);
    ...
    <div className="bg-white p-6 rounded-lg shadow-sm border">
      ...
      <div className="h-64">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart
            data={activeTab === "Orders" ? ordersChartData : getData()}
          ...
          >
            ...
            <XAxis
              dataKey={activeTab === "Orders" ? "label" : getXAxisKey()}
              axisLine={false}
              tickLine={false}
            ...
            <Area
              type="monotone"
              dataKey={activeTab === "Orders" ? "orders" : getDataKey()}
              stroke="#ff6b35"
              strokeWidth={3}
